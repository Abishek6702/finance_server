...
describe("Transport API", () => {
  ...
  it("returns full mapping with admin auth (GET /)", async () => {
    const res = await request(app).get("/api/transport").set(adminAuth());
  ...
  it("rejects stops without auth", async () => {
    const res = await request(app).post("/api/transport/stops").send({});
    expect(res.status).toBe(401);
  });

  it("returns stops without filters", async () => {
    const res = await request(app).post("/api/transport/stops").set(superadminAuth()).send({});
    expect(res.status).toBe(200);
    expect(Array.isArray(res.body.data)).toBe(true);
  });

  it("returns stops by route", async () => {
    const res = await request(app).post("/api/transport/stops").set(superadminAuth()).send({ route: "Bharathiyar University" });
    expect(res.status).toBe(200);
  });

  it("rejects stops route non-string", async () => {
    const res = await request(app).post("/api/transport/stops").set(superadminAuth()).send({ route: 123 });
    expect(res.status).toBe(400);
    expect(res.body.message).toMatch(/route.*string/i);
  });

  it("rejects stops empty route string", async () => {
    const res = await request(app).post("/api/transport/stops").set(superadminAuth()).send({ route: "   " });
    expect(res.status).toBe(400);
  ...
  it("rejects buses without auth", async () => {
    const res = await request(app).post("/api/transport/buses").send({});
    expect(res.status).toBe(401);
  });

  it("rejects buses when stop missing", async () => {
    const res = await request(app).post("/api/transport/buses").set(superadminAuth()).send({});
    expect(res.status).toBe(400);
    expect(res.body.message).toMatch(/stop.*required/i);
  });

  it("rejects buses for blank stop", async () => {
    const res = await request(app).post("/api/transport/buses").set(superadminAuth()).send({ stop: "   " });
    expect(res.status).toBe(400);
  });

  it("rejects buses for non-string stop", async () => {
    const res = await request(app).post("/api/transport/buses").set(superadminAuth()).send({ stop: 42 });
    expect(res.status).toBe(400);
    expect(res.body.message).toMatch(/stop.*string/i);
  });

  it("returns buses for valid stop", async () => {
    const res = await request(app).post("/api/transport/buses").set(superadminAuth()).send({ stop: "Kinathukadavu" });
    expect(res.status).toBe(200);
    expect(Array.isArray(res.body.data)).toBe(true);
  });
  ...
  it("rejects fees without filters", async () => {
    const res = await request(app).post("/api/transport/fees").set(superadminAuth()).send({});
    expect(res.status).toBe(400);
    expect(res.body.message).toMatch(/at least one/i);
  });

  it("rejects fees with non-string busNo", async () => {
    const res = await request(app).post("/api/transport/fees").set(superadminAuth()).send({ busNo: 9 });
    expect(res.status).toBe(400);
  });

  it("rejects fees with empty busNo", async () => {
    const res = await request(app).post("/api/transport/fees").set(superadminAuth()).send({ busNo: "  " });
    expect(res.status).toBe(400);
  });

  it("rejects fees with non-string stop", async () => {
    const res = await request(app).post("/api/transport/fees").set(superadminAuth()).send({ stop: true });
    expect(res.status).toBe(400);
  });

  it("returns fees by busNo", async () => {
    const res = await request(app).post("/api/transport/fees").set(superadminAuth()).send({ busNo: "1" });
    expect(res.status).toBe(200);
    expect(Array.isArray(res.body.data)).toBe(true);
  });

  it("returns fees by stop", async () => {
    const res = await request(app).post("/api/transport/fees").set(superadminAuth()).send({ stop: "Kinathukadavu" });
    expect(res.status).toBe(200);
  });

  it("returns fees by busNo and stop", async () => {
    const res = await request(app).post("/api/transport/fees").set(superadminAuth()).send({ busNo: "1", stop: "Kinathukadavu" });
    expect(res.status).toBe(200);
  });

  /* ─── ADD SINGLE TRANSPORT (Superadmin) ───── */
  ...
  it("rejects add transport with admin auth (must be superadmin)", async () => {
    const res = await request(app)
      .post("/api/transport/add")
      .set(adminAuth())
      .send({ route: "Test Route", busNo: "T1", stop: "TestStop", fee: 5000 });
    expect(res.status).toBe(401);
  });
