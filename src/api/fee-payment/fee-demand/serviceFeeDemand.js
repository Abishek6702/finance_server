...
const buildStudentTypeInfo = (s) => {
 ...
 const getFeeDemandList = async (query = {}) => {
   ...
   const students = await Student.find(studentFilter)
     .select(
      ...
      "transport.isApplicable transport.transport transport.route transport.busNo transport.stop transport.fee " +
      "hostel.isApplicable hostel.hostel hostel.block hostel.sharing hostel.isAttached hostel.fee"
     )
     .lean();
   ...
   const data = students.map((s) => {
     ...
     return {
      ...
      studentType: buildStudentTypeInfo(s),
     };
   });
 ...
 const getFeeDemandByRollNo = async (rollNo, query = {}) => {
   ...
   const feeSummary = yearRecords.map((yr) => {
     ...
     return {
      ...
      studentType: buildStudentTypeInfo(student),
     };
   });
 ...
 /* ────────────────────────────────────────────────
    API 3: GET /FeeDemand/:rollNo/:academicYear
    Semester-level fee breakdown for one academic year
 ──────────────────────────────────────────────── */
 const getFeeDemandBySemester = async (rollNo, academicYear, query = {}) => {
   const includeProfile = query.includeProfile !== "false";
   ...
   const buildSemesterBlock = (type, sem, attachTransport, attachHostel) => {
    ...
    const studentType = buildStudentTypeInfo(student);
