const { Hostel, generateHostelId } = require("./model.hostel");
...
const addHostel = async (data) => {
  ...
  data.id = generateHostelId(data.sharing, data.block, data.isAttached);
...
const bulkAddHostel = async (records) => {
  ...
  for (let i = 0; i < records.length; i++) {
    try {
      ...
      records[i].id = generateHostelId(records[i].sharing, records[i].block, records[i].isAttached);
