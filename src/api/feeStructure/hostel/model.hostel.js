...
const generateHostelId = (sharing, block, isAttached) =>
  `${sharing}${block}${isAttached ? 1 : 0}`;

hostelSchema.pre("validate", async function () {
  if (!this.id) {
    this.id = generateHostelId(this.sharing, this.block, this.isAttached);
  }
});
...
module.exports = { Hostel, seedHostel, generateHostelId };